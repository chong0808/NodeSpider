...
function downloadImg(asyncNum) {
			...
			for(var j=0;j<photos.length;j++){
				...
				if(photos[j].name&&photos[j].nameSrc){
					fs.mkdir(__dirname+'/'+asyncNum.title+'/'+photos[j].name+'('+photos[j].id+')',function(err){
						if(err){console.log(err)}
							console.log('创建目录成功')
					})
				}else{
					fs.readdir(__dirname+'/'+asyncNum.title+'/'+'匿名' , function(err,files){
						if(err){
							console.log(err)
						}
						if(!files.length){
							fs.mkdir(__dirname+'/'+asyncNum.title+'/'+'匿名',function(err){
								if(err){console.log(err)}
					...
					async.mapLimit(photos[j].imgSrc, asyncNum.download_v, function(photo, callback) {
						...
						fetch_data_get(photo, {})
							.then(function(result) {
								...
								fs.writeFile('./'+asyncNum.title+'/'+ name+'('+nameId+')'+'/' + fileName, result.body, function(err) {
